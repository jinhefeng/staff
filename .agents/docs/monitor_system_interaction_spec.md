...
- **Main**：
  - **Top Row**：五个指标卡片（总工单、待执行任务、活跃对话、定时任务、后台延迟项）。
  - **Middle Row (Left 2/3)**：工单详细列表，支持溢出滚动。
  - **Middle Row (Right 1/3)**：上下拆分为两个面板：
    - 上方面板：近期会话活动摘要 (RECENT SESSIONS)。
    - 下方面板：定时任务清单 (CRON JOBS)，展示任务触发条件及实时执行倒计时。
  - **Bottom Row**：系统事件日志摘要 (SYSTEM EVENTS)。
