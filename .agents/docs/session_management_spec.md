...
## 7. 时间感知标签注入 (Temporal Labeling)
为了增强 LLM 对对话近因性的敏感度，`get_history` 阶段会为历史消息注入相对时间标签：
- **注入策略**：将 `timestamp` 转换为相对描述（如 `[刚才]`, `[5分钟内]`, `[1小时前]`），并拼接到 `content` 头部。
- **解析鲁棒性**：系统必须同时兼容 ISO 8601 (含 `T` 分隔符) 与标准 SQL 日期格式，以防历史数据迁移或不同来源导致的解析爆炸。
- **Prompt 优化**：使用分桶逻辑（Binning）降低标签的变化频率，以提高 Prompt Cache 的命中率。
